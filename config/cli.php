...
return function (Application $console) {
    ...
    )->setDescription('Send upcoming song feedback from the AutoDJ back to Radiolize.');
    ...
    )->setDescription(__('Run all general Radiolize setup steps.'));
    ...
    )->setDescription('Trigger regeneration of Radiolize API documentation.');
    ...
    )->setDescription(__('List all settings in the Radiolize settings database.'));

    $console->command(
        'azuracast:settings:set setting-key setting-value',
        Command\Settings\SetCommand::class
    )->setDescription('Set the value of a setting in the Radiolize settings database.');

    $console->command(
        'azuracast:backup [path] [--storage-location-id=] [--exclude-media]',
        Command\Backup\BackupCommand::class
    )->setDescription(__('Back up the Radiolize database and statistics (and optionally media).'));

    $console->command(
        'azuracast:restore path [--restore] [--release]',
        Command\Backup\RestoreCommand::class
    )->setDescription('Restore a backup previously generated by Radiolize.');
};
